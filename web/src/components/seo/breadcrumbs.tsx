 ...
 * Current-page crumbs (no href) omit `item` per Google BreadcrumbList guidance.
 */
export function Breadcrumbs({ crumbs }: { crumbs: Crumb[] }) {
  ...
  const itemListElement = crumbs.map((c, idx) => {
    ...
    if (c.href) {
      entry.item = c.href.startsWith("http") ? c.href : `${root}${c.href.startsWith("/") ? c.href : `/${c.href}`}`;
    }
    return entry;
  });
  ...
  return (
    <>
      <nav aria-label="Breadcrumb" className="text-sm text-muted-foreground" suppressHydrationWarning>
