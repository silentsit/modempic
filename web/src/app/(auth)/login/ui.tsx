...
function LoginFormInner({ socialProviders, callbackUrl: callbackUrlProp, idPrefix = "login" }: LoginFormProps) {
  ...
  return (
    <>
      {registered ? (
        <p className="mt-4 rounded-2xl border border-primary/25 bg-primary-subtle px-4 py-2.5 text-sm text-primary">
      ...
      <form onSubmit={onSubmit} className="mt-6 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between">
            ...
            <Link href="/forgot-password" className="text-xs text-accent transition-colors hover:text-accent-hover hover:underline">
              Forgot password?
            </Link>
          </div>
...
export function LoginForm(props: LoginFormProps) {
  return (
    <Suspense fallback={<div className="mt-6 h-48 animate-pulse rounded-2xl bg-muted" />}>
