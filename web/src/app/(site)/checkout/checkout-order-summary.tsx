...
const promoInputCls =
  "mt-1.5 h-11 rounded-xl border-input bg-card";
            ...
            return (
              <li key={l.id} className="flex gap-3 py-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium leading-snug text-foreground">{l.product.name}</p>
                  {tier ? <p className="mt-0.5 text-xs text-muted-foreground">{tier}</p> : null}
                  <p className="mt-1 text-xs text-muted-foreground">× {l.quantity}</p>
                </div>
                <p className="shrink-0 text-sm font-medium tabular-nums text-foreground">{formatUsd(lineTotal)}</p>
              </li>
            );
          })}
        </ul>
        ...
        <div className="mt-4 border-t border-border pt-4 text-sm">
          <div className="flex flex-col gap-2">
            <div className="flex justify-between text-muted-foreground">
              <span>Subtotal</span>
              <span className="tabular-nums text-foreground">{formatUsd(subtotalCents)}</span>
            </div>
            {discountCents > 0 ? (
              <div className="flex justify-between text-muted-foreground">
                <span>Discount</span>
                <span className="tabular-nums text-primary">-{formatUsd(discountCents)}</span>
              </div>
            ) : null}
            <div className="flex justify-between text-muted-foreground">
              <span>Shipping</span>
            ...
            <div className="flex justify-between text-muted-foreground">
              <span>Tax</span>
              <span className="tabular-nums text-foreground">{formatUsd(taxCents)}</span>
            </div>
          ...
          <div id="coupon" className="space-y-1.5 border-t border-border pt-4">
            <label htmlFor="couponCode" className="block text-sm font-medium text-muted-foreground">
              Have a promo code?
            </label>
            <Input
              form={checkoutFormId}
